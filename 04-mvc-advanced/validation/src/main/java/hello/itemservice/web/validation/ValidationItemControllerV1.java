...
public class ValidationItemControllerV1 {
  ...
  public String addItem(@ModelAttribute Item item, RedirectAttributes redirectAttributes, Model model) {
    ...
    if (price == null || price < 1000 || price > 1000000) {
      errors.put("price", "가격은 1,000 ~ 1,000,000원 사이까지 허용됩니다.");
    }

    Integer quantity = item.getQuantity();
    if (quantity == null || quantity >= 9999) {
      errors.put("quantity", "최대 허용 수량은 9,999개 입니다.");
    }
    ...
    if (price != null && quantity != null) {
      ...
      if (resultPrice < minSalePrice) {
        errors.put("globalError", "가격  * 수량의 합은" + minSalePrice + "이상이어야 합니다. 현재 값 = " + resultPrice);
      }
    }
    // 검증 실패 시, 다시 입력 폼으로
