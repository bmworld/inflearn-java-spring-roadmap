...
//  private TraceId traceIdHolder; // TraceId 동기화 [!] 동기화, 동시성 이슈 있음.
  private ThreadLocal<TraceId> traceIdHolder = new ThreadLocal<>(); // TraceId 동기화 [!] 동기화, 동시성 이슈 있음.
  ...
  private TraceId syncTraceId() {
    ...
    if (traceId == null) {
      ...
      traceIdHolder.set(traceId.createNextId());; // level increase by 1
    }
  ...
  private void releaseTraceId() {
    ...
    if (traceId.isFirstLevel()) {
      traceIdHolder.remove(); // ! [중요] 해당 Thread의 Local 값(전용 보관소) 초기화.
    } else {
